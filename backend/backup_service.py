            ...
            for test in tests:
                records_sheet.append(
                    [
                        record.get("id", ""),
                        record.get("name", ""),
                        record.get("age_years", record.get("age", "")),
            ...
            for test in tests:
                records_sheet.append(
                    [
                        ...
                        sample.get("transport_condition", ""),
                        "Yes"
...
def _backup_local_now():
    return datetime.now(ZoneInfo(os.environ.get("BACKUP_TIMEZONE", "Asia/Kolkata")))


async def _run_daily_backup_if_due(db, *, scheduled=False):
    """Run at most one automatic backup per local date, including after a free-instance wake."""
    if os.environ.get("ENABLE_GOOGLE_DRIVE_BACKUP", "false").lower() != "true":
        return
    local_now = _backup_local_now()
    hour = int(os.environ.get("BACKUP_HOUR", "14"))
    minute = int(os.environ.get("BACKUP_MINUTE", "0"))
    if (local_now.hour, local_now.minute) < (hour, minute):
        return
    day = local_now.date().isoformat()
    async with _catchup_lock:
        # A successful manual backup today also satisfies the daily backup requirement.
        day_start = local_now.replace(hour=0, minute=0, second=0, microsecond=0)
        day_start_utc = day_start.astimezone(timezone.utc).isoformat()
        latest = await db.backup_logs.find_one(
            {"status": "success", "created_at": {"$gte": day_start_utc}},
            {"_id": 0, "created_at": 1},
        )
        if latest:
            return
        # Atomic insert prevents concurrent workers from starting duplicate uploads.
        lease_id = "daily:" + day
        try:
            await db.backup_run_locks.insert_one({
                "_id": lease_id, "created_at": now_iso(),
            })
        except DuplicateKeyError:
            return
        try:
            await perform_backup(db, "scheduled" if scheduled else "catchup")
        except Exception as exc:
            # Allow a later request to retry; avoid blocking a whole day after an error.
            await db.backup_run_locks.delete_one({"_id": lease_id})
            await db.backup_settings.update_one(
                {"key": "google_drive"},
                {"$set": {"last_backup_at": now_iso(),
                          "last_backup_status": "failed", "last_error": str(exc),
                          "updated_at": now_iso()}},
                upsert=True,
            )


async def _catchup_on_activity(db):
    global _catchup_check_at, _catchup_task
    now = datetime.now(timezone.utc)
    if _catchup_check_at and (now - _catchup_check_at).total_seconds() < 300:
        return
    if _catchup_task is not None and not _catchup_task.done():
        return
    _catchup_check_at = now
    _catchup_task = asyncio.create_task(_run_daily_backup_if_due(db))
...
def register_backup(app, router, db, get_current_user, require_master_access):
    ...
    async def status(user=Depends(require_master_access)):
        ...
        return {
            ...
            "schedule": (
                f"{int(os.environ.get('BACKUP_HOUR', '14')):02d}:"
            ...
            "last_successful_automatic_backup": (
                (await db.backup_logs.find_one(
                    {"status": "success", "trigger": {"$in": ["scheduled", "catchup"]}},
                    {"_id": 0, "created_at": 1},
                    sort=[("created_at", -1)],
                ) or {}).get("created_at")
            ),
    ...
    @app.middleware("http")
    async def backup_wake_check(request: Request, call_next):
        if request.url.path.startswith("/api/") and not request.url.path.startswith("/api/backup/"):
            await _catchup_on_activity(db)
