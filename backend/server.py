...
def _samples_from_payload(
    ...
    for raw in raw_samples:
        ...
        configured_mappings = (dataset_doc or {}).get("sample_mappings") or []
        if configured_mappings and not mapping:
            raise HTTPException(
                status_code=400,
                detail=f"Sample type '{sample_type}' is not configured for this dataset",
            )
...
async def create_record(payload: LabRecordCreate, user=Depends(get_current_user)):
    ...
    for sample in samples:
        if not sample.get("lab_number"):
            sample["lab_number"] = await generate_lab_number(dataset_doc["key"])
