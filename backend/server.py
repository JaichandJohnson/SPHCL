...
async def _samples_from_payload(
    ...
    for raw in raw_samples:
        ...
        if configured_mappings and not mapping:
            raise HTTPException(
                status_code=400,
                detail=f"Sample type '{sample_type}' is not configured for {dataset_doc.get('name')}",
            )

        tests = [_test_dict(test) for test in (sample.get("tests") or [])]
        if not tests and mapping and mapping.get("auto_assign"):
            tests = [_test_dict({"test": name}) for name in (mapping.get("tests") or [])]
        if not tests:
            raise HTTPException(
                status_code=400,
                detail=f"At least one test is required for sample: {sample_type}",
            )

        tests = _validate_tests(tests)
        if mapping:
            allowed = {str(name).strip().lower() for name in (mapping.get("tests") or []) if str(name).strip()}
            invalid = [item["test"] for item in tests if item["test"].lower() not in allowed]
            if invalid:
                raise HTTPException(
                    status_code=400,
    ...
    # Temporary single-sample compatibility.
    sample_type = str(data.get("sample_type") or "").strip()
    tests = _tests_from_payload(payload)
    if sample_type and tests:
        dataset_doc = await get_dataset(fallback_dataset)
        return [{
            "id": f"sample_{uuid.uuid4().hex[:12]}",
            "dataset": dataset_doc["key"],
            "lab_number": data.get("lab_number"),
            "epid_number": data.get("epid_number"),
            "sample_type": sample_type,
            "tests": _validate_tests(tests),
            "remarks": None,
        }]
...
def _serialize_record(doc: dict) -> dict:
    ...
    if not doc.get("samples") and (doc.get("sample_type") or doc.get("tests")):
        ...
        for sample in doc.get("samples") or []:
            ...
            sample.setdefault("dataset", doc.get("dataset") or "routine")
            sample.setdefault("lab_number", "")
            sample.setdefault("epid_number", None)
            sample["tests"] = [_test_dict(t) for t in (sample.get("tests") or [])]
...
async def list_panels(dataset: Optional[str] = None, active: Optional[bool] = None, user=Depends(get_current_user)):
    query: Dict[str, Any] = {}
    if dataset:
        clauses.append({"$or": [{"dataset": normalize_key(dataset)}, {"samples.dataset": normalize_key(dataset)}]})
...
async def save_panel(payload: PanelMaster, user=Depends(get_current_user)):
...
async def delete_panel(panel_id: str, user=Depends(get_current_user)):
...
def _apply_filters(query: dict, dataset: Optional[str], test: Optional[str], district: Optional[str], sample_type: Optional[str], result_contains: Optional[str], search: Optional[str], date_from: Optional[str], date_to: Optional[str]):
    ...
    if search:
        ...
        clauses.append({"$or": [
            {"name": regex}, {"lab_number": regex}, {"samples.lab_number": regex}, {"patient_id": regex},
            {"requesting_institution": regex}, {"epid_number": regex}, {"samples.epid_number": regex},
...
async def create_record(payload: LabRecordCreate, user=Depends(get_current_user)):
    ...
    samples = await _samples_from_payload(payload, dataset_doc["key"])
    if not samples:
...
@api_router.put("/records/{rid}/results")
async def update_individual_results(
    rid: str,
    payload: IndividualResultUpdate,
    user=Depends(get_current_user),
):
    existing = await db.records.find_one({"id": rid}, {"_id": 0})
    if not existing:
    ...
    wrapper = LabRecordUpdate(
        samples=payload.samples,
        remarks=payload.remarks,
        dataset=existing.get("dataset") or "routine",
    )
    samples = await _samples_from_payload(wrapper, existing.get("dataset") or "routine")
    existing_by_id = {str(s.get("id")): s for s in (existing.get("samples") or [])}
    for sample in samples:
        previous = existing_by_id.get(str(sample.get("id")))
        if previous and not sample.get("lab_number"):
            sample["lab_number"] = previous.get("lab_number")
        if not sample.get("lab_number"):
            sample["lab_number"] = await generate_lab_number(sample["dataset"])
