...
async def update_record(rid: str, payload: LabRecordUpdate, user=Depends(get_current_user)):
    ...
    if sample_fields.intersection(raw):
        ...
        # Normal record editing cannot alter stored results or result dates.
        for sample in samples:
            previous_sample = existing_by_id.get(
            ...
            previous_by_id = {
                ...
                for item in (previous_sample.get("tests") or [])
                if item.get("id")
            }
            previous_by_name = {
            ...
            for item in sample.get("tests") or []:
                previous = (
                    previous_by_id.get(str(item.get("id") or ""))
                    or previous_by_name.get(str(item.get("test") or ""))
                )
                if not previous:
                    continue
                item["result1"] = previous.get("result1") or ""
                item["result2"] = previous.get("result2") or ""
                item["result_date"] = previous.get("result_date")
