...
async def generate_lab_number(dataset_key: str) -> str:
    """Generate a dataset-specific number that restarts each calendar year."""
    ds = await get_dataset(dataset_key)
    year = datetime.now(timezone.utc).year
    counter_key = f"lab_{ds['key']}_{year}"
    ...
    assigned = int(counter.get("value", 1))
    prefix = ds.get("prefix") or ds["key"].upper()
    short_year = str(year)[-2:]

    # Example: MDS 001/26. A new counter key is used in 2027, so it
    # automatically starts at MDS 001/27 without any manual reset.
    return f"{prefix} {assigned:03d}/{short_year}"
