...
async def generate_pertussis_patient_number() -> tuple[int, int]:
    year = datetime.now(timezone.utc).year
    counter = await db.counters.find_one_and_update(
        {"key": f"pertussis_patient_{year}"},
        {
            "$inc": {"value": 1},
            "$setOnInsert": {
                "key": f"pertussis_patient_{year}",
...
def assign_pertussis_lab_numbers(
    ...
    for sample in samples:
        if sample.get("dataset") == "pertussis":
            sample["lab_number"] = (
                f"{pertussis_sample_prefix(sample.get('sample_type'))} "
                f"{sequence}"
            )
...
async def create_record(payload: LabRecordCreate, user=Depends(get_current_user)):
    ...
    if any(
        ...
        pertussis_number, pertussis_year = (
