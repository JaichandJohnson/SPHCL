...
async def save_panel(payload: PanelMaster, user=Depends(get_current_user)):
    ...
    tests = list(dict.fromkeys([str(x).strip() for x in payload.tests if str(x).strip()]))
    if not name:
    ...
    panel_id = payload.id or f"panel_{uuid.uuid4().hex[:12]}"
    duplicate = await db.panels.find_one({"dataset": dataset_key, "name": {"$regex": f"^{re.escape(name)}$", "$options": "i"}, "id": {"$ne": panel_id}}, {"_id": 0})
    if duplicate:
        raise HTTPException(status_code=409, detail="A panel with this name already exists")
    doc = {"id": panel_id, "name": name, "dataset": dataset_key, "tests": tests, "active": payload.active, "updated_at": now_iso()}
    await db.panels.update_one({"id": panel_id}, {"$set": doc, "$setOnInsert": {"created_at": now_iso()}}, upsert=True)
    return doc
...
async def test_statistics_report(date_from: Optional[str] = None, date_to: Optional[str] = None, dataset: Optional[str] = None, district: Optional[str] = None, user=Depends(get_current_user)):
    ...
    stats={}
    for raw in records:
        for item in (_serialize_record(raw).get("tests") or []):
