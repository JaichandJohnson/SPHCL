...
from backup_service import register_backup, perform_backup
...
DEFAULT_DATASETS = [
    ...
    {"key": "typhoid_surveillance", "name": "Typhoid Surveillance", "prefix": "TSP", "next_number": 1, "active": True},
]
...
VALID_RESULT_VALUES = {"", "Positive", "Negative", "Indeterminate", "Other", "Pending"}
...
class TatThresholdPayload(BaseModel):
    dataset: str
    test: str
...
class TatThresholdBulkItem(BaseModel):
    test: str
    threshold_days: Optional[float] = None
    item_type: str = "test"


class TatThresholdBulkPayload(BaseModel):
    dataset: str
    items: List[TatThresholdBulkItem] = Field(default_factory=list)
...
class LabRecord(BaseModel):
    ...
    pertussis_patient_number: Optional[int] = None
    pertussis_sequence_year: Optional[int] = None
...
class AdminResultCorrectionItem(BaseModel):
    sample_id: str
...
class AdminResultCorrectionPayload(BaseModel):
    corrections: List[AdminResultCorrectionItem] = Field(default_factory=list)
        ...
        samples.append(
            {
                ...
                "nvhcp_program": bool(sample.get("nvhcp_program", False)),
                "sample_quality": (
                    str(sample.get("sample_quality") or "").strip()
                    or None
                ),
                "transport_condition": (
                    str(sample.get("transport_condition") or "").strip()
                    or None
                ),
...
def validate_sample_conditions(samples: List[Dict[str, Any]]) -> None:
    for sample in samples:
        dataset = normalize_key(sample.get("dataset") or "")
        if dataset not in CONDITION_REQUIRED_DATASETS:
            continue
        if not str(sample.get("sample_quality") or "").strip():
            raise HTTPException(
                status_code=400,
                detail="Sample Quality is required for this dataset",
            )
        if not str(sample.get("transport_condition") or "").strip():
            raise HTTPException(
                status_code=400,
                detail="Transport Condition is required for this dataset",
            )
...
async def generate_mr_patient_number() -> tuple[int, int]:
    """
    Allocate one MR patient number from Dataset Master next_number.

    This allows the laboratory to set the current running number in
    Master Data before real-data entry. sequence_year is initialized
    without resetting an existing manually configured next_number; in a
    subsequent calendar year the sequence resets to 1 automatically.
    """
    year = datetime.now(timezone.utc).year
    dataset = await get_dataset("mr_surveillance")

    sequence_year = dataset.get("sequence_year")
    if sequence_year is None:
        await db.datasets.update_one(
            {"key": dataset["key"]},
            {"$set": {"sequence_year": year}},
        )
    elif int(sequence_year) != year:
        await db.datasets.update_one(
            {"key": dataset["key"]},
            {
                "$set": {
    ...
    assigned = int(updated.get("next_number", 2)) - 1
...
def pertussis_sample_prefix(sample_type: str) -> str:
    normalized = re.sub(
        r"\s+",
        " ",
        str(sample_type or "").strip().lower(),
    )
    ...
    raise HTTPException(
        status_code=400,
        detail=(
            "Pertussis patient numbering supports Serum and "
            "Nasopharyngeal Swab"
        ),
    )


async def generate_pertussis_patient_number() -> tuple[int, int, str]:
    """
    Allocate exactly one number per Pertussis patient using the Pertussis
    Dataset Master counter. This keeps Pertussis numbering synchronized with
    the configured WP prefix and next_number instead of using a separate
    counter collection.
    """
    dataset = await get_dataset("pertussis")
    ...
    if updated and "next_number" in updated:
        sequence = int(updated["next_number"]) - 1
        prefix = str(
            updated.get("prefix") or dataset.get("prefix") or "WP"
        ).strip()
    else:
        sequence = int(dataset.get("next_number", 1))
        prefix = str(dataset.get("prefix") or "WP").strip()
        await db.datasets.update_one(
            {"key": dataset["key"]},
            {"$inc": {"next_number": 1}},
        )

    return sequence, datetime.now(timezone.utc).year, prefix


def assign_pertussis_lab_numbers(
    samples: List[Dict[str, Any]],
    sequence: int,
    master_prefix: str = "WP",
) -> None:
    base_prefix = str(master_prefix or "WP").strip()

    for sample in samples:
        if sample.get("dataset") != "pertussis":
            continue

        normalized = re.sub(
            r"\s+",
            " ",
            str(sample.get("sample_type") or "").strip().lower(),
        )

        if normalized == "serum":
            sample_prefix = f"{base_prefix} S"
        elif normalized in {
            "nasopharyngeal swab",
            "naso pharyngeal swab",
        }:
            sample_prefix = f"{base_prefix} NP"
        else:
            # Reuse the validation/error message already defined for
            # Pertussis sample types.
            pertussis_sample_prefix(sample.get("sample_type"))
            continue

        sample["lab_number"] = f"{sample_prefix} {sequence}"
...
async def generate_rabies_patient_number() -> tuple[int, int]:
    """
    Allocate one Rabies patient number from Dataset Master next_number.

    This keeps RC/RB/RT/RS/RCS/RP/RSR numbering synchronized with the
    number configured by the administrator in Master Data.
    """
    year = datetime.now(timezone.utc).year
    dataset = await get_dataset("rabies")
...
def assign_rabies_lab_numbers(samples: List[Dict[str, Any]], sequence: int, year: int) -> None:
    """Assign one patient sequence with the Rabies specimen code attached to R."""
    year_suffix = str(year)[-2:]
    for sample in samples:
        if sample.get("dataset") == "rabies":
            specimen_suffix = rabies_sample_suffix(sample.get("sample_type"))
            sample["lab_number"] = f"R{specimen_suffix} {sequence:03d}/{year_suffix}"
...
@api_router.get("/lab-number/preview")
async def preview_lab_number(
    dataset: str,
    user=Depends(get_current_user),
):
    key = normalize_key(dataset)
    ds = await get_dataset(key)
    year = datetime.now(timezone.utc).year
    year_suffix = str(year)[-2:]

    if key == "mr_surveillance":
        sequence = int(ds.get("next_number", 1))
        return {
            "display": f"WM S/U/T {sequence}",
            "note": "One patient sequence; prefix depends on specimen",
        }
    ...
    if key == "rabies":
        ...
        return {
            "display": (
                "RC/RB/RT/RS/RCS/RP/RSR "
                f"{sequence:03d}/{year_suffix}"
            ),
            "note": "One patient sequence; code depends on specimen",
        }

    if key == "pertussis":
        sequence = int(ds.get("next_number", 1))
        prefix = str(ds.get("prefix") or "WP").strip()
        return {
            "display": f"{prefix} S/NP {sequence}",
            "note": "S=Serum, NP=Nasopharyngeal Swab",
        }

    sequence = int(ds.get("next_number", 1))
    prefix = str(ds.get("prefix") or "").strip()
    return {
        "display": f"{prefix} {sequence}".strip(),
        "note": "",
    }
...
async def seed_defaults():
    ...
    await db.records.create_index("district")
    await db.records.create_index("requesting_institution")
    ...
    try:
        await db.tat_thresholds.drop_index("dataset_1_test_1")
    except Exception:
        pass
    await db.tat_thresholds.create_index(
        [("dataset", 1), ("item_type", 1), ("test", 1)],
        unique=True,
    )
...
async def google_login(payload: GoogleLoginPayload, response: Response):
    ...
    # Multiple browser windows/computers may legitimately use the same
    # authorized Google account. Do not invalidate the user's other active
    # sessions when a new login occurs. Only remove expired sessions.
...
async def require_master_access(
    ...
    """
    Validate the temporary Master Data unlock against the authenticated
    application session.

    The previous implementation additionally required a
    `master_access_token` cookie. Because the frontend and backend are on
    different Render subdomains, some browsers block that cookie as a
    cross-site/third-party cookie. The normal authenticated session is already
    carried reliably via Authorization: Bearer, so the admin unlock is stored
    server-side against that session token instead.
    """
    session_token = _request_session_token(request)
    if not session_token:
        raise HTTPException(
            status_code=403,
            detail="Master Data is locked",
        )
    ...
    if not expires_at or expires_at <= datetime.now(timezone.utc):
        await db.master_sessions.delete_many(
            {"session_token": session_token}
        )
        raise HTTPException(
            status_code=403,
            detail="Master Data access expired",
        )
...
async def unlock_master_data(
    ...
    return {
        "ok": True,
        "unlocked_until": expires_at.isoformat(),
    }
...
async def lock_master_data(
    ...
    # Delete the legacy cookie too, so older browser state is harmless.
    response.delete_cookie(
        "master_access_token",
        path="/",
        samesite="none",
...
# Backup & Restore uses the same administrative lock/session as Master Data.
register_backup(
    app,
    api_router,
    db,
...
@api_router.put("/records/{rid}/admin-results")
async def admin_correct_results(
    rid: str,
    payload: AdminResultCorrectionPayload,
    ...
    for correction in payload.corrections:
        sample = next(
            (
                ...
                if str(value.get("id")) == correction.sample_id
            ),
            None,
        )
        if not sample:
        ...
        test_item = next(
            (
                value for value in (sample.get("tests") or [])
                if (
                    correction.test_id
                    and str(value.get("id")) == correction.test_id
                )
                or str(value.get("test") or "") == correction.test
            ),
            None,
        )
        if not test_item:
        ...
        test_item["result1"] = correction.result1 or ""
        test_item["result2"] = correction.result2 or ""
        test_item["result_date"] = correction.result_date or None
        updated += 1
...
# ---------- Configurable test reference information ----------
class TestReferencePayload(BaseModel):
    dataset: str
    test: str
    normal_range: str = ""
    reference_remarks: str = ""
    unit: str = ""
    display_on_report: bool = True


@api_router.get("/test-references")
async def list_test_references(user=Depends(get_current_user)):
    return await db.test_references.find({}, {"_id": 0}).to_list(5000)


@api_router.post("/test-references")
async def save_test_reference(payload: TestReferencePayload, user=Depends(require_master_access)):
    dataset = normalize_key(payload.dataset)
    dataset_doc = await get_dataset(dataset)
    test = payload.test.strip()
    if not test or test.casefold() not in {str(t).casefold() for t in dataset_doc.get("tests", [])}:
        raise HTTPException(status_code=400, detail="Select a test belonging to the dataset")
    doc = {
        "dataset": dataset, "test": test,
        "normal_range": payload.normal_range.strip(),
        "reference_remarks": payload.reference_remarks.strip(),
        "unit": payload.unit.strip(),
        "display_on_report": payload.display_on_report,
        "updated_at": now_iso(),
    }
    await db.test_references.update_one(
        {"dataset": dataset, "test": test}, {"$set": doc}, upsert=True
    )
    return doc
...
async def create_record(payload: LabRecordCreate, user=Depends(get_current_user)):
    ...
    validate_sample_conditions(samples)
    ...
    pertussis_number = None
    pertussis_year = None
    if any(
        sample.get("dataset") == "pertussis"
        for sample in samples
    ):
        (
            pertussis_number,
            pertussis_year,
            pertussis_master_prefix,
        ) = await generate_pertussis_patient_number()

        assign_pertussis_lab_numbers(
            samples,
            pertussis_number,
            pertussis_master_prefix,
        )
    ...
    rec = LabRecord(
        ...
        pertussis_patient_number=pertussis_number,
        pertussis_sequence_year=pertussis_year,
        samples=samples,
...
async def list_records(
    ...
    nvhcp_program: Optional[bool] = None,
    page: int = 1,
    page_size: int = 25,
    user=Depends(get_current_user),
):
    ...
    if nvhcp_program is not None:
        query.setdefault("$and", []).append(
            {"samples": {"$elemMatch": {"nvhcp_program": nvhcp_program}}}
        )
    if pending is True:
        pending_test = {
            "$or": [
...
class ReportRemarksPayload(BaseModel):
    report_remarks: str = ""


@api_router.put("/records/{rid}/report-remarks")
async def save_report_remarks(rid: str, payload: ReportRemarksPayload, user=Depends(get_current_user)):
    # A report-specific field: never overwrite clinical sample/test remarks.
    text = payload.report_remarks.strip()
    if len(text) > 5000:
        raise HTTPException(status_code=400, detail="Report remarks must not exceed 5000 characters")
    result = await db.records.update_one(
        {"id": rid},
        {"$set": {"report_remarks": text, "report_remarks_updated_at": now_iso(),
                  "report_remarks_updated_by": user["user_id"]}},
    )
    if not result.matched_count:
        raise HTTPException(status_code=404, detail="Record not found")
    return {"id": rid, "report_remarks": text}


# Capture the reference information once, when the individual report is first issued.
# An empty snapshot is intentional: later Master Data changes must not populate it.
@api_router.post("/records/{rid}/issue-reference-snapshot")
async def issue_reference_snapshot(rid: str, user=Depends(get_current_user)):
    record = await db.records.find_one({"id": rid}, {"_id": 0})
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")
    if "report_reference_snapshot" in record:
        return {"report_reference_snapshot": record["report_reference_snapshot"],
                "report_reference_issued_at": record.get("report_reference_issued_at")}

    # Include hidden reference settings as well, to preserve their display flag.
    references = await db.test_references.find({}, {"_id": 0}).to_list(5000)
    by_key = {(str(ref.get("dataset") or "").casefold(),
               str(ref.get("test") or "").strip().casefold()): ref for ref in references}
    snapshot = []
    seen = set()
    samples = record.get("samples") or [{"dataset": record.get("dataset"),
                                         "tests": record.get("tests") or []}]
    for sample in samples:
        dataset = str(sample.get("dataset") or record.get("dataset") or "").casefold()
        for test in sample.get("tests") or []:
            test_name = str(test.get("test") or test.get("name") or "").strip()
            key = (dataset, test_name.casefold())
            if not test_name or key in seen:
                continue
            seen.add(key)
            ref = by_key.get(key)
            if ref:
                snapshot.append({"dataset": dataset, "test": test_name,
                                 "normal_range": ref.get("normal_range", ""),
                                 "unit": ref.get("unit", ""),
                                 "reference_remarks": ref.get("reference_remarks", ""),
                                 "display_on_report": ref.get("display_on_report", True)})

    # Atomic first-issue wins, including when two users print simultaneously.
    issued_at = now_iso()
    updated = await db.records.find_one_and_update(
        {"id": rid, "report_reference_snapshot": {"$exists": False}},
        {"$set": {"report_reference_snapshot": snapshot,
                  "report_reference_issued_at": issued_at,
                  "report_reference_issued_by": user["user_id"]}},
        return_document=ReturnDocument.AFTER, projection={"_id": 0})
    if updated is None:
        updated = await db.records.find_one({"id": rid}, {"_id": 0})
    return {"report_reference_snapshot": updated.get("report_reference_snapshot", []),
            "report_reference_issued_at": updated.get("report_reference_issued_at")}
...
def _previous_sample_for_update(
    sample: Dict[str, Any],
    index: int,
    ...
    """
    Match an edited sample to its stored sample without using sample type as
    identity. Sample type itself is editable.
    """
    sample_id = str(sample.get("id") or "")
    if sample_id and sample_id in existing_by_id:
        return existing_by_id[sample_id]

    lab_number = str(sample.get("lab_number") or "").strip()
    if lab_number:
        for previous in existing_samples:
            if str(previous.get("lab_number") or "").strip() == lab_number:
                return previous

    # Defensive fallback for edit screens that accidentally recreate the
    # sample object but retain the same sample order.
    if 0 <= index < len(existing_samples):
        return existing_samples[index]

    return None


def _sequence_from_mr_lab_number(value: str) -> Optional[int]:
    match = re.search(r"(\d+)\s*$", str(value or "").strip())
    return int(match.group(1)) if match else None


def _sequence_from_rabies_lab_number(
    value: str,
...
def _sequence_from_pertussis_lab_number(
    value: str,
) -> Optional[int]:
    match = re.search(r"(\d+)\s*$", str(value or "").strip())
    return int(match.group(1)) if match else None
...
async def update_record(rid: str, payload: LabRecordUpdate, user=Depends(get_current_user)):
    ...
    if sample_fields.intersection(raw):
        ...
        # Reattach the existing assigned number before any numbering logic.
        # Correcting sample type must never make an existing sample consume a
        # new sequence.
        for sample_index, sample in enumerate(samples):
            previous = _previous_sample_for_update(
                sample,
                sample_index,
                existing_samples,
                existing_by_id,
            ...
            previous_dataset = normalize_key(
                previous.get("dataset")
                or existing.get("dataset")
                or ""
            )
            current_dataset = normalize_key(
                sample.get("dataset")
                or dataset_doc["key"]
            )
        ...
        if any(
            ...
            if not mr_number:
                for previous in existing_samples:
                    if previous.get("dataset") != "mr_surveillance":
                        continue
                    mr_number = _sequence_from_mr_lab_number(
                        previous.get("lab_number")
                    )
                    if mr_number:
                        break
            ...
            if not mr_number:
                ...
                mr_year = datetime.now(timezone.utc).year
        ...
        if any(sample.get("dataset") == "rabies" for sample in samples):
            if not rabies_number:
                for previous in existing_samples:
                    if previous.get("dataset") != "rabies":
                        continue
                    (
                        rabies_number,
                        recovered_year,
                    ) = _sequence_from_rabies_lab_number(
                        previous.get("lab_number")
                    )
                    if rabies_number:
                        rabies_year = (
            ...
            if not rabies_number:
                rabies_number, rabies_year = await generate_rabies_patient_number()
            elif not rabies_year:
                rabies_year = datetime.now(timezone.utc).year
        ...
        pertussis_number = existing.get("pertussis_patient_number")
        pertussis_year = existing.get("pertussis_sequence_year")
        if any(
            sample.get("dataset") == "pertussis"
            for sample in samples
        ):
            if not pertussis_number:
                for previous in existing_samples:
                    if previous.get("dataset") != "pertussis":
                        continue
                    pertussis_number = _sequence_from_pertussis_lab_number(
                        previous.get("lab_number")
                    )
                    if pertussis_number:
                        break
            ...
            if not pertussis_number:
                ...
                pertussis_dataset = await get_dataset("pertussis")
                pertussis_master_prefix = str(
                    pertussis_dataset.get("prefix") or "WP"
                ).strip()

            assign_pertussis_lab_numbers(
                samples,
                int(pertussis_number),
                pertussis_master_prefix,
            )
            update["pertussis_patient_number"] = int(
                pertussis_number
            )
            update["pertussis_sequence_year"] = int(
                pertussis_year
            )
        ...
        for sample_index, sample in enumerate(samples):
            ...
            if (
                sample.get("dataset")
                not in {"mr_surveillance", "rabies", "pertussis"}
                and not sample.get("lab_number")
                and previous
                and normalize_key(previous.get("dataset") or "")
                == normalize_key(sample.get("dataset") or "")
            ):
                sample["lab_number"] = previous.get("lab_number")
            ...
            if (
                ...
                # Only a genuinely new sample reaches this point.
                sample["lab_number"] = await generate_lab_number(
                    sample.get("dataset") or dataset_doc["key"]
                )
...
async def delete_record(
    ...
    record = await db.records.find_one(
        {"id": rid},
        {"_id": 0, "id": 1, "name": 1, "lab_number": 1},
    )
    if not record:
        raise HTTPException(status_code=404, detail="Record not found")

    result = await db.records.delete_one({"id": rid})
    if result.deleted_count != 1:
        raise HTTPException(
            status_code=500,
            detail="Record could not be deleted",
        )
    ...
    return {
        ...
        "lab_number": record.get("lab_number"),
    }
...
async def import_records(file: UploadFile = File(...), user=Depends(get_current_user)):
    ...
    current_group = None
    current_key = None
    panel_cache: Dict[tuple, Optional[Dict[str, Any]]] = {}

    async def resolve_import_panel(dataset_key: str, name: str):
        clean_name = " ".join(str(name or "").split()).strip()
        cache_key = (normalize_key(dataset_key), clean_name.lower())
        if cache_key in panel_cache:
            return panel_cache[cache_key]

        candidates = await db.panels.find(
            {"active": {"$ne": False}},
            {"_id": 0},
        ).to_list(5000)
        ...
        for candidate in candidates:
            if (
                normalize_key(candidate.get("dataset") or "")
                == normalize_key(dataset_key)
                and " ".join(
                    str(candidate.get("name") or "").split()
                ).strip().lower()
                == clean_name.lower()
            ):
                match = candidate
    ...
    for idx, row in df.iterrows():
        try:
            ...
            panel = await resolve_import_panel(
                dataset_doc["key"], test_name
            )

            if panel is None and test_name.strip().lower().endswith("panel"):
                raise ValueError(
                    f"Panel '{test_name}' was not found as an active panel "
                    f"for dataset '{dataset_doc.get('name') or dataset_doc['key']}'. "
                    "Check Panel Master name, dataset, active status, and component tests."
                )
            try:
            ...
            if panel:
                ...
                panel_name = str(panel.get("name") or test_name).strip()
                panel_tests = [
                    str(value).strip()
                    for value in (panel.get("tests") or [])
                    if str(value).strip()
                ]
                if not panel_tests:
                    raise ValueError(
                        f"Panel '{panel_name}' has no component tests"
                    )

                if any(
                    str(p.get("panel_name") or "").lower()
                    == panel_name.lower()
                    for p in current_group["assigned_panels"]
                ):
                    raise ValueError(
                        f"Duplicate panel in same consecutive group: {panel_name}"
                    )

                current_group["assigned_panels"].append({
                    "panel_id": panel_id,
                ...
                existing = {
                    str(t.get("test") or "").lower(): t
                    for t in current_group["tests"]
                }
                for component in panel_tests:
                    source = {
                        "type": "panel",
                        "id": panel_id,
                        "name": panel_name,
                    }
                    key_name = component.lower()
                    if key_name in existing:
                        sources = existing[key_name].setdefault(
                            "sources", []
                        )
                        if not any(
                            s.get("type") == "panel"
                        ...
                        current_group["tests"].append(item)
                        existing[key_name] = item
            else:
                duplicate = next(
                    (
                        t for t in current_group["tests"]
                        if str(t.get("test") or "").lower()
                        == test_name.lower()
                    ),
                    None,
                )
                if duplicate:
                    sources = duplicate.setdefault("sources", [])
                    if (
                        any(s.get("type") == "panel" for s in sources)
                        and not any(
                            s.get("type") != "panel" for s in sources
                        )
                    ):
                        sources.append({
                            "type": "manual",
                        ...
                        duplicate["result1"] = test_item["result1"]
                        duplicate["result2"] = test_item["result2"]
                        duplicate["result_date"] = test_item["result_date"]
                    else:
                        raise ValueError(
                            f"Duplicate test in same consecutive group: {test_name}"
                        )
                else:
                    test_item["sources"] = [{
                        "type": "manual",
    ...
    for group in groups:
        try:
            ...
            sample = {
                ...
                "tests": _validate_tests(group["tests"]),
                "assigned_panels": group.get("assigned_panels", []),
                "remarks": None,
            }
            samples = [sample]
        ...
        for sample in samples:
            ...
            for test in tests:
                rows.append({
                    ...
                    "Age Years": record.get("age_years", record.get("age", "")) if first_row else "",
                    "Age Months": record.get("age_months", "") if first_row else "",
                    "Age Days": record.get("age_days", "") if first_row else "",
                    ...
                    "Sample Quality": sample.get("sample_quality", ""),
                    "Transport Condition": sample.get("transport_condition", ""),
                    "Test": test.get("test", ""),
...
@api_router.get("/tat-thresholds")
async def list_tat_thresholds(
    dataset: Optional[str] = None,
    user=Depends(get_current_user),
):
    query: Dict[str, Any] = {}
    ...
    docs = await db.tat_thresholds.find(
        query,
        {"_id": 0},
    ).sort([("dataset", 1), ("item_type", 1), ("test", 1)]).to_list(5000)
    for doc in docs:
        doc.setdefault("item_type", "test")
    return docs


@api_router.post("/tat-thresholds")
async def save_tat_threshold(
    payload: TatThresholdPayload,
    user=Depends(require_master_access),
):
    dataset = normalize_key(payload.dataset)
    await get_dataset(dataset)

    test = str(payload.test or "").strip()
    if not test:
        raise HTTPException(status_code=400, detail="Test is required")
    if payload.threshold_days < 0:
        raise HTTPException(
            status_code=400,
            detail="TAT threshold cannot be negative",
        )

    item_type = str(payload.item_type or "test").strip().lower()
    if item_type not in {"test", "panel"}:
        raise HTTPException(
            status_code=400,
            detail="TAT item type must be test or panel",
        )
    ...
    doc = {
        ...
        "test": test,
        "item_type": item_type,
        "threshold_days": float(payload.threshold_days),
        "updated_at": datetime.now(timezone.utc),
    }
    await db.tat_thresholds.update_one(
        {
            "dataset": dataset,
            "test": test,
            "item_type": item_type,
...
@api_router.post("/tat-thresholds/bulk")
async def save_tat_thresholds_bulk(
    payload: TatThresholdBulkPayload,
    user=Depends(require_master_access),
):
    dataset = normalize_key(payload.dataset)
    await get_dataset(dataset)
    ...
    for item in payload.items:
        name = str(item.test or "").strip()
        item_type = str(item.item_type or "test").strip().lower()

        if not name or item_type not in {"test", "panel"}:
            continue
        ...
        if item.threshold_days is None:
            result = await db.tat_thresholds.delete_one(key)
            deleted += result.deleted_count
            continue

        if item.threshold_days < 0:
            raise HTTPException(
                status_code=400,
                detail=f"TAT threshold cannot be negative: {name}",
            )
        ...
        await db.tat_thresholds.update_one(
            key,
            {
                "$set": {
                    **key,
                    "threshold_days": float(item.threshold_days),
                    "updated_at": now,
                }
            },
            upsert=True,
...
@api_router.delete("/tat-thresholds")
async def delete_tat_threshold(
    dataset: str,
    test: str,
    item_type: str = "test",
    user=Depends(require_master_access),
):
    result = await db.tat_thresholds.delete_one(
        {
            "dataset": normalize_key(dataset),
            "test": str(test or "").strip(),
            "item_type": str(item_type or "test").strip().lower(),
        }
    )
    return {"ok": True, "deleted": result.deleted_count}
...
async def tat_report(
    ...
    projection = {
        ...
        "tests": 1,
        "samples.dataset": 1,
        "samples.lab_number": 1,
        "samples.tests": 1,
        "samples.assigned_panels": 1,
    }
    records = await db.records.find(
        query,
        projection,
    ).sort("date", 1).to_list(25000)
    ...
    for raw in records:
        ...
        if samples:
            for sample in samples:
                sample_dataset = sample.get("dataset") or record.get("dataset")
                if dataset_key and sample_dataset != dataset_key:
                    continue
                for item in sample.get("tests") or []:
                    if test and item.get("test") != test:
                        continue
                    tat = _tat_days(
                        record.get("date"),
                    ...
                    points.append(
                        {
                            ...
                            "lab_number": sample.get("lab_number")
                            or record.get("lab_number"),
                ...
                for panel in sample.get("assigned_panels") or []:
                    ...
                    panel_tests = panel.get("tests") or []
                    if not panel_name or not panel_tests:
                        continue

                    result_dates = []
                    complete = True
                    for test_name in panel_tests:
                        item = test_by_name.get(test_name)
                        if not item or not item.get("result_date"):
                            complete = False
                            break
                        result_dates.append(item.get("result_date"))

                    if not complete or not result_dates:
                        continue
            ...
            if dataset_key and record.get("dataset") != dataset_key:
                continue
    ...
    values = sorted([point["tat_days"] for point in points])
    median = None
    if values:
        middle = len(values) // 2
        median = (
...
def _statistics_bucket(result: Any) -> str:
...
def _panel_statistics_bucket(component_tests: List[Dict[str, Any]]) -> str:
    """
    Treat a panel as one reporting/statistics entity.

    Priority:
    - Any positive component -> Positive panel.
    - Otherwise any pending/unentered component -> Pending panel.
    - Otherwise any indeterminate component -> Indeterminate panel.
    - Otherwise all completed components negative -> Negative panel.
    """
    if not component_tests:
        return "pending"

    buckets = [
        _statistics_bucket(item.get("result1"))
    ...
    if "pending" in buckets:
        return "pending"
    if "indeterminate" in buckets:
        return "indeterminate"
    return "negative"


def _add_statistics_row(
...
def _sample_panel_test_names(
    sample: Dict[str, Any],
) -> Dict[str, set]:
    panels = {}
    for panel in sample.get("assigned_panels") or []:
        panel_id = str(panel.get("panel_id") or "")
        panel_name = str(panel.get("panel_name") or "").strip()
        if not panel_name:
            continue
        panel_tests = {
            str(value).strip()
...
# Monthly summary: receipt and completion cohorts are intentionally independent.
def _monthly_bucket(value):
    value = str(value or "").strip().lower()
    return value if value in {"positive", "negative", "indeterminate", "other"} else "pending"


async def _monthly_summary_data(month: str, selected: List[str]):
    if not re.fullmatch(r"\d{4}-(0[1-9]|1[0-2])", month):
        raise HTTPException(status_code=422, detail="Month must be YYYY-MM")
    start = month + "-01"
    year, month_number = map(int, month.split("-"))
    end = f"{year + (month_number == 12):04d}-{(month_number % 12) + 1:02d}-01"
    configured = await db.datasets.find({}, {"_id": 0, "key": 1, "name": 1}).to_list(1000)
    names = {d["key"]: d.get("name") or d["key"] for d in configured}
    keys = set(selected or names)
    unknown = keys - set(names)
    if unknown:
        raise HTTPException(status_code=422, detail="Unknown dataset: " + ", ".join(sorted(unknown)))
    thresholds = await db.tat_thresholds.find({}, {"_id": 0}).to_list(10000)
    limits = {(d.get("dataset"), d.get("item_type") or "test", d.get("test")): d.get("threshold_days") for d in thresholds}
    def fresh(key):
        return {"dataset": key, "name": names.get(key, key), "received": 0, "completed": 0,
                "pending": 0, "rejected": 0, "tat_assessed": 0, "tat_within": 0,
                "tat_exceeded": 0, "tat_days_sum": 0, "tat_without_threshold": 0}
    rows = {key: fresh(key) for key in sorted(keys)}
    tests = {}
    # Query both cohorts: older receipts may be completed this month.
    query = {"$or": [{"date": {"$gte": start, "$lt": end}},
                      {"samples.tests.result_date": {"$gte": start, "$lt": end}},
                      {"tests.result_date": {"$gte": start, "$lt": end}}]}
    async for record in db.records.find(query, {"_id": 0}):
        received_date = str(record.get("date") or "")[:10]
        samples = record.get("samples") or [{"dataset": record.get("dataset"),
                                               "tests": record.get("tests") or [],
                                               "assigned_panels": []}]
        for sample in samples:
            key = sample.get("dataset") or record.get("dataset")
            if key not in rows:
                continue
            row = rows[key]
            received = start <= received_date < end
            items = sample.get("tests") or []
            panel_defs = _sample_panel_test_names(sample)
            completed_in_month = any(start <= str(t.get("result_date") or "")[:10] < end for t in items)
            if received:
                row["received"] += 1
                if not any(t.get("result_date") for t in items):
                    row["pending"] += 1
                if str(sample.get("status") or "").lower() == "rejected":
                    row["rejected"] += 1
            if completed_in_month:
                row["completed"] += 1
            panel_components = {name for panel in panel_defs.values() for name in panel["tests"]}
            entities = [(t.get("test"), "test", [t]) for t in items
                        if t.get("test") and (t.get("test") not in panel_components or
                        any(src.get("type") != "panel" for src in t.get("sources") or []))]
            for panel in panel_defs.values():
                components = [t for t in items if t.get("test") in panel["tests"]]
                entities.append((panel["name"], "panel", components))
            for name, kind, components in entities:
                if not components:
                    continue
                dates = [str(t.get("result_date") or "")[:10] for t in components]
                complete = all(dates) and all(_monthly_bucket(t.get("result1")) != "pending" for t in components)
                finished = max(dates) if complete else None
                if received:
                    test_row = tests.setdefault((key, kind, name), {"dataset": key, "dataset_name": names[key],
                        "item_type": kind, "test": name, "assigned": 0, "positive": 0, "negative": 0,
                        "indeterminate": 0, "other": 0, "pending": 0})
                    test_row["assigned"] += 1
                    bucket = (_panel_statistics_bucket(components) if kind == "panel" else
                              _monthly_bucket(components[0].get("result1")))
                    test_row[bucket if bucket in test_row else "pending"] += 1
                if not finished or not start <= finished < end:
                    continue
                tat = _tat_days(received_date, finished)
                if tat is None or tat < 0:
                    continue
                limit = limits.get((key, kind, name))
                if limit is None:
                    row["tat_without_threshold"] += 1
                    continue
                row["tat_assessed"] += 1
                row["tat_days_sum"] += tat
                if tat <= float(limit):
                    row["tat_within"] += 1
                else:
                    row["tat_exceeded"] += 1
    for row in rows.values():
        n = row["tat_assessed"]
        row["average_tat_days"] = round(row["tat_days_sum"] / n, 2) if n else None
        row["tat_compliance_percent"] = round(100 * row["tat_within"] / n, 1) if n else None
    totals = fresh("all")
    totals["name"] = "Total (selected datasets)"
    for row in rows.values():
        for field in ("received", "completed", "pending", "rejected", "tat_assessed", "tat_within",
                      "tat_exceeded", "tat_days_sum", "tat_without_threshold"):
            totals[field] += row[field]
    n = totals["tat_assessed"]
    totals["average_tat_days"] = round(totals["tat_days_sum"] / n, 2) if n else None
    totals["tat_compliance_percent"] = round(100 * totals["tat_within"] / n, 1) if n else None
    return {"month": month, "generated_at": datetime.now(timezone.utc).isoformat(),
            "datasets": sorted(keys), "rows": list(rows.values()), "totals": totals,
            "testing": sorted(tests.values(), key=lambda r: (r["dataset_name"], r["item_type"], r["test"]))}


@api_router.get("/reports/monthly-summary")
async def monthly_summary(month: str, datasets: Optional[str] = None, user=Depends(get_current_user)):
    return await _monthly_summary_data(month, [v for v in (datasets or "").split(",") if v])


@api_router.get("/reports/monthly-summary/export")
async def monthly_summary_export(month: str, datasets: Optional[str] = None, user=Depends(get_current_user)):
    data = await _monthly_summary_data(month, [v for v in (datasets or "").split(",") if v])
    import openpyxl
    from openpyxl.styles import Font, PatternFill, Alignment
    from openpyxl.utils import get_column_letter
    book = openpyxl.Workbook()
    book.remove(book.active)
    sheets = [("Dataset Summary", data["rows"] + [data["totals"]],
               [("Dataset", "name"), ("Received", "received"), ("Completed", "completed"),
                ("Pending at month end*", "pending"), ("Rejected", "rejected"),
                ("TAT assessed", "tat_assessed"), ("Within TAT", "tat_within"),
                ("Exceeded TAT", "tat_exceeded"), ("Average TAT (days)", "average_tat_days"),
                ("TAT compliance (%)", "tat_compliance_percent"),
                ("Without threshold", "tat_without_threshold")]),
              ("Testing Summary", data["testing"],
               [("Dataset", "dataset_name"), ("Type", "item_type"), ("Test / Panel", "test"),
                ("Assigned to received samples", "assigned"), ("Positive", "positive"),
                ("Negative", "negative"), ("Indeterminate", "indeterminate"),
                ("Other", "other"), ("Pending", "pending")])]
    for title, entries, columns in sheets:
        ws = book.create_sheet(title)
        ws.append(["MDS LIMS — Monthly Laboratory Summary", "Month: " + month])
        ws.append(["Datasets: " + ", ".join(data["datasets"])])
        ws.append([heading for heading, _ in columns])
        for item in entries:
            ws.append([item.get(field) for _, field in columns])
        ws.freeze_panes = "A4"
        ...
        for col, (heading, _) in enumerate(columns, 1):
            ws.column_dimensions[get_column_letter(col)].width = min(40, max(16, len(heading) + 3))
        ws.row_dimensions[3].height = 34
    notes = book.create_sheet("Definitions")
    for line in ["Monthly report definitions", "Received: sample receipt date in selected month.",
                 "Completed: samples with at least one result date in selected month (not necessarily fully completed).",
                 "Pending: received in month with no test result date recorded at report generation.",
                 "Testing summary: test/panel results for samples received in selected month.",
                 "TAT: completed test/panel results in month with a configured threshold; measured in calendar days.",
                 "Average TAT is weighted across assessed test/panel results; pending and missing thresholds excluded.",
                 "Reports reflect current record state; not a locked month-end snapshot."]:
        notes.append([line])
    ...
    return StreamingResponse(output, media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": f'attachment; filename="MDS_LIMS_Monthly_{month}.xlsx"'})
...
async def test_statistics_report(
    ...
    raw_records = await db.records.find(
        query,
        {"_id": 0},
    ).to_list(100000)
    ...
    for raw in raw_records:
        ...
        for sample in record.get("samples") or []:
            if (
                dataset
                and normalize_key(sample.get("dataset") or "")
                != normalize_key(dataset)
            ):
                continue

            tests = sample.get("tests") or []
            panels = _sample_panel_test_names(sample)

            # Count each assigned panel exactly once for this sample.
            for panel in panels.values():
                panel_name = panel["name"]
                panel_test_names = panel["tests"]
                ...
                _add_statistics_row(
                    ...
                    panel_name,
                    "panel",
                    _panel_statistics_bucket(component_tests),
                )

            # Individual test statistics are separate. A test that exists only
            # because it belongs to a panel does NOT increment its individual
            # test statistic.
            ...
            for item in tests:
                ...
                has_panel_source = any(
                    source.get("type") == "panel"
                    for source in sources
                )
                has_non_panel_source = any(
                    source.get("type") != "panel"
                    for source in sources
                )
                ...
                panel_only = (
                    has_panel_source
                    and not has_non_panel_source
                ) or (
                    not sources
                    and name in panel_component_names
                )
    ...
    for row in stats.values():
        ...
        row["positivity_rate"] = (
            round(row["positive"] / completed * 100, 2)
            if completed
            else 0
        )
        rows.append(row)
    ...
    rows.sort(
        key=lambda item: (
            0 if item["item_type"] == "panel" else 1,
            item["test"].lower(),
        )
    )
    ...
    return {
        "items": rows,
        "total_entities": sum(item["total"] for item in rows),
    }


@api_router.get("/reports/test-statistics/export")
async def export_test_statistics(
    date_from: Optional[str] = None,
    date_to: Optional[str] = None,
    dataset: Optional[str] = None,
    district: Optional[str] = None,
    ...
    result = await test_statistics_report(
        date_from=date_from,
        date_to=date_to,
        dataset=dataset,
        district=district,
        ...
        rows.append(
            {
                "Type": (
                    "Panel"
                    if item.get("item_type") == "panel"
                    else "Individual Test"
                ),
                "Test / Panel": item.get("test", ""),
                "Total": item.get("total", 0),
                "Positive": item.get("positive", 0),
                "Negative": item.get("negative", 0),
                "Indeterminate": item.get("indeterminate", 0),
                "Pending": item.get("pending", 0),
                "Positivity Rate (%)": item.get(
                    "positivity_rate",
                    0,
                ),
            }
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": (
                "attachment; "
                f"filename=MDS_LIMS_Test_Statistics_"
...
async def clear_operational_data(
    ...
    """
    Clear transactional/test laboratory data while preserving all Master Data
    and system configuration.

    Safety sequence:
    1. Require the shared Master Data administrator unlock.
    2. Require the exact destructive-action confirmation.
    3. Create a fresh Google Drive pre-clear backup.
    4. Delete laboratory records.
    5. Reset every operational sequence/counter so real-data numbering starts
       again from 1.
    """
    if confirm != "DELETE TEST DATA":
        raise HTTPException(
            status_code=400,
            detail='Confirmation must be exactly: DELETE TEST DATA',
        )

    # Do not clear anything unless a recoverable safety backup succeeds.
    # Retention cleanup is skipped so this backup cannot remove an older
    # recovery point during the destructive operation itself.
    try:
        safety_backup = await perform_backup(
            db,
            "pre_clear_test_data",
            apply_cleanup=False,
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "Test data was NOT deleted because the mandatory "
                f"pre-clear backup failed: {exc}"
            ),
        ) from exc
    ...
    # The counters collection contains generated operational sequences such as:
    # patient_YYYY, mr_patient_YYYY and rabies_patient_YYYY.
    counters_deleted = (
        await db.counters.delete_many({})
    ).deleted_count

    # Normal lab numbers and Pertussis patient numbering consume the Dataset
    # Master's next_number. Reset only that operational sequence field while
    # preserving dataset names, prefixes, tests, mappings and configuration.
    dataset_reset = await db.datasets.update_many(
        {},
        {"$set": {"next_number": 1}},
    )

    # Record the destructive administrative action in backup_logs, which is
    # deliberately preserved.
    await db.backup_logs.insert_one(
        {
            "id": str(uuid.uuid4()),
            "created_at": datetime.now(timezone.utc).isoformat(),
            "status": "test_data_cleared",
            "trigger": "admin_clear_test_data",
            "records_deleted": records_deleted,
            "counters_deleted": counters_deleted,
            "datasets_reset": dataset_reset.modified_count,
            "safety_backup": safety_backup.get("filename"),
            "safety_excel_backup": safety_backup.get(
                "excel_filename"
            ),
        }
    )
    ...
    return {
        ...
        "numbering_restarts_from": 1,
        "preserved": [
            "datasets and dataset prefixes",
            "tests and sample types",
            "sample-test mappings",
            "panels",
            "institutions",
            "TAT thresholds",
            "users",
            "backup settings and Google Drive connection",
            "backup history/logs",
