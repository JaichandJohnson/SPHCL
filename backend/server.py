...
async def list_options(dataset: Optional[str] = None, user=Depends(get_current_user)):
    ...
    grouped: Dict[str, Any] = {"test": [], "tests_by_dataset": {}, "district": [], "sample_type": [], "datasets": datasets}
...
async def add_option(item: OptionItem, user=Depends(get_current_user)):
    ...
    if item.type == "test":
        doc["dataset"] = normalize_key(item.dataset or "routine")
