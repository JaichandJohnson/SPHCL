...
async def require_master_access(
    ...
    session = await db.user_sessions.find_one(
        {"session_token": token},
        {"_id": 0, "master_unlocked_until": 1},
    )
    unlocked_until = (session or {}).get("master_unlocked_until")

    if isinstance(unlocked_until, str):
        try:
            unlocked_until = datetime.fromisoformat(unlocked_until)
        except ValueError:
            unlocked_until = None

    if unlocked_until and unlocked_until.tzinfo is None:
        unlocked_until = unlocked_until.replace(tzinfo=timezone.utc)

    if not unlocked_until or unlocked_until <= datetime.now(timezone.utc):
        raise HTTPException(
            status_code=403,
...
async def master_status(
    ...
    if isinstance(unlocked_until, datetime):
        unlocked_until = unlocked_until.isoformat()

    unlocked = False
    if unlocked_until:
        try:
            expiry = datetime.fromisoformat(unlocked_until)
            if expiry.tzinfo is None:
                expiry = expiry.replace(tzinfo=timezone.utc)
            unlocked = expiry > datetime.now(timezone.utc)
        except ValueError:
            unlocked = False

    return {
        "unlocked": unlocked,
        "unlocked_until": unlocked_until if unlocked else None,
    }
...
async def unlock_master_data(
    ...
    if not configured_password:
        raise HTTPException(
            status_code=503,
            detail="Master Data password is not configured",
        )
    ...
    unlocked_until = (
        ...
        + timedelta(minutes=MASTER_UNLOCK_MINUTES)
    )

    await db.user_sessions.update_one(
        {"session_token": token},
        {"$set": {"master_unlocked_until": unlocked_until.isoformat()}},
    )
