...
def _samples_from_payload(payload: LabRecordCreate | LabRecordUpdate, dataset_doc: Optional[dict] = None) -> List[Dict[str, Any]]:
    data = payload.model_dump(exclude_unset=True)
    ...
    for raw in raw_samples:
        ...
        tests = [_test_dict(t) for t in (sample.get("tests") or [])]
        if not tests and data.get("auto_assign_tests") and dataset_doc:
            tests = [_test_dict({"test": name}) for name in (dataset_doc.get("tests") or DEFAULT_TESTS_BY_DATASET.get(dataset_doc.get("key"), []))]
        if not tests:
            raise HTTPException(status_code=400, detail=f"At least one test is required for sample: {sample_type}")
        samples.append({
...
async def delete_option(type: str, value: str, dataset: Optional[str] = None, user=Depends(get_current_user)):
    ...
    if type == "test":
        mapped_dataset = await db.datasets.find_one({"tests": value}, {"_id": 0, "name": 1})
    ...
    if type == "sample_type":
        mapped_dataset = await db.datasets.find_one({"sample_types": value}, {"_id": 0, "name": 1})
        if mapped_dataset:
...
async def create_record(payload: LabRecordCreate, user=Depends(get_current_user)):
    ...
    samples = _samples_from_payload(payload, dataset_doc)
    if not samples:
...
def _flatten_records_for_export(records: List[dict]) -> List[dict]:
    ...
    for record in records:
        ...
        for sample in samples:
            ...
            for test in tests:
                rows.append({
                    "Lab Number": record.get("lab_number", "") if first_row else "",
