...
export default function ReportPrint() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 py-5 print:bg-white print:py-0">
      ...
      {type === "individual" ? (
        record ? (
          <IndividualDocument record={record} datasets={datasets} />
        ) : (
  ...
  return (
    <header className="border-b-2 border-black pb-3 text-center">
      <div className="flex items-center justify-center gap-4">
        ...
        />
        <div>
          <div className="text-[17px] font-bold uppercase tracking-wide">
            State Public Health and Clinical Laboratory, Trivandrum
          </div>
          <div className="mt-1 text-[14px] font-semibold uppercase">
...
function IndividualDocument({ record, datasets }) {
    ...
    <main className="report-sheet mx-auto min-h-[297mm] w-[210mm] bg-white p-[14mm] text-[12px] text-black shadow-lg">
      ...
      <section className="mt-7 space-y-6">
        {samples.map((sample, sampleIndex) => (
          ...
          >
            <div className="mb-2 grid grid-cols-3 border border-black">
              <InfoCell
                label="Specimen No."
            ...
            <table className="w-full border-collapse border border-black">
              <thead>
                <tr>
                  ...
                  <th className="w-32 border border-black px-3 py-2 text-left">
                    Result Date
                  </th>
                </tr>
              </thead>
...
function DistrictDocument({
  ...
  const datasetName =
    datasets.find((item) => item.key === dataset)?.name ||
    (dataset ? dataset : "All Datasets");
  const title =
    dataset === "mr_surveillance"
      ? "Measles / Rubella Surveillance Sample Report"
      : dataset === "typhoid_surveillance"
        ? "Typhoid Surveillance Consolidated Report"
        : "District Consolidated Laboratory Report";

  return (
    <main className="report-sheet mx-auto min-h-[297mm] w-[210mm] bg-white p-[10mm] text-[10px] text-black shadow-lg">
      <Header title={title} />

      <section className="mt-4 grid grid-cols-4 gap-3 border border-black p-3">
        <Info label="District" value={district || "—"} />
        <Info label="Dataset" value={datasetName} />
        <Info label="From" value={fmt(dateFrom)} />
        <Info label="To" value={fmt(dateTo)} />
      </section>
      ...
      <table className="mt-4 w-full border-collapse border border-black">
        ...
        <tbody>
          {rows.map(({ record, sample, test }, index) => (
