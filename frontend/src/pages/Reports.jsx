...
export default function Reports() {
  ...
  const [tatThreshold, setTatThreshold] = useState(
    () => Number(localStorage.getItem("mds_tat_threshold_days") || 3)
  );
      ...
      {tab === "individual" ? (
        ...
        <>
          ...
          {loading ? (
            ...
            <TatReport
              data={tat}
              threshold={tatThreshold}
              setThreshold={(value) => {
                setTatThreshold(value);
                localStorage.setItem(
                  "mds_tat_threshold_days",
                  String(value)
                );
              }}
            />
          ...
          <>
            ...
            <Field label="Tests">
              <select
                ...
                className="min-h-28 w-full rounded border bg-white p-2"
              >
...
function TatReport({ data, threshold, setThreshold }) {
  const groups = new Map();
  (data.points || []).forEach((point) => {
    const name = point.test || "Unknown Test";
    const values = groups.get(name) || [];
    values.push(Number(point.tat_days || 0));
  ...
  return (
    <div className="space-y-4">
      <Card className="border p-4 shadow-none">
        <Field label="Acceptable TAT Threshold (days)">
          <Input
            type="number"
            min="0"
            step="0.5"
        ...
        <div className="mt-2 text-xs text-slate-500">
          Stored on this browser until changed.
        </div>
      </Card>
      <Card className="overflow-hidden border shadow-none">
        <table className="w-full text-sm">
