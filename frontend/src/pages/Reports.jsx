...
export default function Reports() {
  const [opts, setOpts] = useState({ dataset: [], test: [], district: [], sample_type: [] });
                ...
                return tests.map((t, index) => {
                  ...
                  return (
                    <tr key={`${r.id}-${index}`} className={first ? "border-t border-slate-200" : "border-t border-slate-50"}>
                      ...
                      <td className="px-3 py-2 tabular-nums">{first ? r.date : ""}</td>
                      ...
                      <td className="px-3 py-2 tabular-nums text-slate-500">{t.result_date}</td>
                    </tr>
                  );
                });
              })}
...
const FilterSelect = ({ label, value, onChange, options, testId }) => (
  <Field label={label}>
    ...
    >
      ...
      {options?.map((o) => (
        <option key={o.value || o.name || o} value={o.value || o.name || o}>
          {o.label || o.name || o}
        </option>
      ))}
    </select>
