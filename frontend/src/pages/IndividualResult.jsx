...
// Compare the numeric lab sequence first; use prefix only for matching numbers.
const compareLabNumbers = (left, right) => {
  const parse = (value) => {
    const text = String(value || "").trim();
    const match = text.match(/^(.*?)\s*(\d+)(?:\s*\/\s*(\d+))?\s*$/);
    return match ? { prefix: match[1].trim(), number: Number(match[2]), year: match[3] ? Number(match[3]) : null, text } : { prefix: text, number: null, year: null, text };
  };
  const a = parse(left);
  const b = parse(right);
  if (a.number !== null && b.number !== null) {
    if (a.number !== b.number) return a.number - b.number;
    const prefix = a.prefix.localeCompare(b.prefix, undefined, { sensitivity: "base" });
...
const TYPHOID_RESULT_OPTIONS = [...RESULT_OPTIONS, "Other"];
...
export default function IndividualResult() {
  ...
  const rows = useMemo(() => {
    ...
    return flattenRecords(records, pendingKeys)
      ...
      const comparison =
        sort.column === "labNumber" ? compareLabNumbers(x, y) :
  ...
  const panelSampleOptions = useMemo(() => {
    ...
    return options.sort((left, right) => {
      ...
      return String(left.labNumber || "").localeCompare(
        String(right.labNumber || ""),
        undefined,
  ...
  const availablePanels = useMemo(() => {
    ...
    const matchingPanels = (panelsByDataset[selectedPanelSample.dataset] || [])
      .filter((panel) =>
    ...
    return matchingPanels.filter((panel) => {
      const signature = [
        String(panel.name || "").trim().toLowerCase(),
        ...(panel.tests || [])
          .map((test) => String(test || "").trim().toLowerCase())
          .sort(),
      ].join("::");

      if (seenPanels.has(signature)) return false;
  ...
  const applyPanelResultDate = (value) => {
    setPanelResultDate(value);

    if (!value || !selectedPanelSample || !panelId) return;

    const panel = availablePanels.find((item) => item.id === panelId);
    if (!panel) return;
    ...
    setRecords((current) =>
      current.map((record) => {
        if (record.id !== selectedPanelSample.recordId) return record;
        ...
        (sample.tests || []).forEach((test) => {
          ...
          if (panelTests.has(test.test) && hasResult) {
            test.result_date = value;
            changed = true;
          }
        });
    ...
    setRecords((current) =>
      current.map((record) => {
        ...
        if (
          ...
          const isSelectedPanelTest =
            selectedPanelSample &&
            row.recordId === selectedPanelSample.recordId &&
            row.sampleIndex === selectedPanelSample.sampleIndex &&
            selectedPanelTests.has(test.test);

          test.result_date = isSelectedPanelTest
            ? panelResultDate || new Date().toISOString().slice(0, 10)
            : new Date().toISOString().slice(0, 10);
        }
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-5">
            ...
            >
              ...
              {panelSampleOptions.map((item) => (
                <option key={item.key} value={item.key}>
                  {displayDate(item.date)} · {item.labNumber} · {item.name} · {item.sampleType}
                </option>
              ))}
            </select>
          </Field>
          ...
          <Field label="Result Date">
            <Input
              ...
              onChange={(event) =>
                applyPanelResultDate(event.target.value)
              }
            />
          </Field>
                ...
                visibleRows.map((row) => (
                  ...
                  >
                    ...
                    <td className="min-w-48 px-3 py-2">
                      <Input
                        list={row.dataset === "typhoid_surveillance" ? "typhoid-result-options" : "individual-result-options"}
                        value={row.result1}
                        onChange={(event) =>
                          updateRow(
                            row,
      ...
      <datalist id="typhoid-result-options">
        {TYPHOID_RESULT_OPTIONS.map((result) => (
