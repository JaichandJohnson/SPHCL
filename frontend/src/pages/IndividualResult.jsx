...
export default function IndividualResult() {
  ...
  const [panelResult, setPanelResult] = useState("Negative");
  ...
  const applyPanelResult = (resultValue) => {
    ...
    const result = String(resultValue || panelResult).trim();
    if (!result) {
      toast.error("Enter a result");
      return;
    }
    ...
    if (!matchingTestIndexes.length) {
      toast.info("No pending tests from this panel were found");
      return;
    }
    ...
    toast.success(
      `${result} applied to ${matchingTestIndexes.length} pending panel test${
        matchingTestIndexes.length === 1 ? "" : "s"
      }. Review exceptions, then click Save Changes.`
    );
  };
      ...
      <Card className="border border-slate-200 bg-white p-4 shadow-none">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-xs text-slate-500">
            Apply one result to all pending tests in a selected sample panel.
            The rows remain visible so exceptions can be changed before Save Changes.
          </p>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 lg:grid-cols-6">
          ...
          <Field label="Result">
            <Input
              ...
              value={panelResult}
              onChange={(event) => setPanelResult(event.target.value)}
          ...
          <div className="flex items-end gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={() => applyPanelResult("Negative")}
              disabled={!panelId}
              className="flex-1"
            >
              Mark All Negative
            ...
            <Button
              type="button"
              onClick={() => applyPanelResult(panelResult)}
              disabled={!panelId || !panelResult.trim()}
