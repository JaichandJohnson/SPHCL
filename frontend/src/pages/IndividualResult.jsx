...
const ALL_DATASET = { key: "", label: "All Datasets" };
...
export default function IndividualResult() {
  ...
  const [datasets, setDatasets] = useState([ALL_DATASET]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDatasets([ALL_DATASET, ...mappedDatasets]);
      setRecords(loadedRecords);

      const loadedPendingKeys = new Set();
      ...
      setPendingKeys(loadedPendingKeys);
  ...
  const rows = useMemo(() => {
    ...
    return flattenRecords(records, pendingKeys).sort((a, b) => {
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      setPendingKeys((current) => {
        ...
        rows.forEach((row) => {
          if (!savedRecordIds.has(row.recordId)) return;
