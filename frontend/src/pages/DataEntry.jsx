...
export default function DataEntry() {
  ...
  const updateSample = (sampleIndex, key, value) => {
    setForm((current) => {
      ...
      samples[sampleIndex] = { ...samples[sampleIndex], [key]: value };
  ...
  const applyDefaultTests = (checked) => {
    setForm((current) => ({
      ...current,
      auto_assign_tests: checked,
      samples: checked
        ? current.samples.map((sample) => ({
            ...
            tests:
              sample.tests.length > 0
                ? sample.tests
                : testOptions.map((test) => newTest(test)),
          }))
        : current.samples,
    }));
  };
    ...
    try {
      ...
      const payload = {
        ...
        epid_number: EPID_DATASETS.has(form.dataset)
          ...
          : null,
        auto_assign_tests: form.auto_assign_tests,
        samples,
        ...
        <form onSubmit={save} className="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <div className="md:col-span-2">
            <label className="inline-flex items-center gap-2 text-sm text-slate-700">
              <input
                type="checkbox"
                checked={form.auto_assign_tests}
                onChange={(event) => applyDefaultTests(event.target.checked)}
              />
              Automatically assign the dataset's default tests to empty samples
            </label>
          </div>
