...
export default function DataEntry() {
  ...
  const selectableItems = useMemo(() => {
    ...
    const permittedPanelIds = new Set(
    ...
    (
      ...
      if (!permittedPanelIds.has(panel.id)) return;
      if (!panel.id || uniquePanels.has(panel.id)) return;
      uniquePanels.set(panel.id, {
        key: `panel::${panel.id}`,
        name: panel.name,
        kind: "panel",
    ...
    const panels = [...uniquePanels.values()].sort((a, b) =>
  ...
  const addSelection = (value) => {
    ...
    setSampleDraft((current) => {
      ...
      if (value.startsWith("panel::")) {
        ...
        if (!permitted.has(panelId)) {
          toast.error(
            "This panel is not mapped to the selected sample type"
          );
          return current;
        }
