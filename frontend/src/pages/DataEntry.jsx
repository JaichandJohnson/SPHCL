...
const DATASETS = [
  ...
  { key: "typhoid_surveillance", label: "Typhoid Surveillance", prefix: "TSP" },
];
...
export default function DataEntry() {
  ...
  const [nextLabNumber, setNextLabNumber] = useState(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [optionsResponse, masterResponse] = await Promise.all([
          api.get("/options"),
          api.get("/master/status"),
        ]);
        setMasterUnlocked(Boolean(masterResponse.data?.unlocked));
        setOpts((current) => ({
          ...current,
        ...
        if (id) {
          ...
          setForm({
            ...
            date: record.date || "",
            name: record.name || "",
            age_years: record.age_years ?? record.age ?? "",
  ...
  useEffect(() => {
    if (id || !defaultDataset) {
      setNextLabNumber(null);
      return;
    }

    let cancelled = false;
    api
      .get("/lab-number/preview", {
        params: { dataset: defaultDataset },
      })
      .then((response) => {
        if (!cancelled) setNextLabNumber(response.data);
      })
      .catch(() => {
        if (!cancelled) setNextLabNumber(null);
      });
  ...
  const conditionRequiredDatasets = new Set([
    "mr_surveillance",
    "diphtheria",
    "pertussis",
  ]);

  const sampleSupportsNvhcp = (sample) =>
  ...
  const saveSampleDraft = () => {
    ...
    const normalizedSampleQuality = String(
      sampleDraft.sample_quality || ""
    ).trim();
    const normalizedTransportCondition = String(
      sampleDraft.transport_condition || ""
    ).trim();

    if (
      conditionRequiredDatasets.has(sampleDraft.dataset) &&
      !normalizedSampleQuality
    ) {
      toast.error("Sample Quality is required for this dataset");
      return;
    }

    if (
      conditionRequiredDatasets.has(sampleDraft.dataset) &&
      !normalizedTransportCondition
    ) {
      toast.error("Transport Condition is required for this dataset");
      return;
    }
    ...
    const savedDraft = {
      ...sampleDraft,
      sample_quality: normalizedSampleQuality,
      transport_condition: normalizedTransportCondition,
      nvhcp_program: sampleSupportsNvhcp(sampleDraft)
        ? Boolean(sampleDraft.nvhcp_program)
        : false,
      epid_number:
  ...
  const save = async (event) => {
    ...
    try {
      const samples = form.samples.map((sample) => ({
        ...
        transport_condition: sample.transport_condition || null,
        tests: sample.tests.map((test) => ({
          id: test.id,
          test: test.test,
      ...
      const payload = {
        ...
        age:
          ...
          form.age_days === ""
            ? null
            : Number(form.age_years || 0),
        age_years:
          form.age_years === "" ? null : Number(form.age_years),
        age_months:
          form.age_months === "" ? null : Number(form.age_months),
        age_days:
          form.age_days === "" ? null : Number(form.age_days),
        sex: form.sex || null,
      ...
      if (id && masterUnlocked) {
        await api.put(`/records/${id}/admin-results`, {
          corrections: form.samples.flatMap((sample) =>
      ...
      api
        ...
        .then((previewResponse) =>
          setNextLabNumber(previewResponse.data)
        )
        .catch(() => {});
      window.setTimeout(() => nameRef.current?.focus(), 0);
      ...
      <Card className="rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-sm">
        ...
        >
          <div className="md:col-span-2">
            ...
            {!id && nextLabNumber && (
              <div className="mt-2 inline-flex flex-wrap items-center gap-x-2 rounded-lg border border-blue-100 bg-blue-50 px-3 py-2 text-sm">
                <span className="font-medium text-slate-600">
                  Next auto-assigned lab number:
                </span>
                <span className="font-semibold text-blue-700">
                  {nextLabNumber.display}
                </span>
                {nextLabNumber.note && (
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div>
                <Input
                  ...
                  placeholder="Days"
                />
                <div className="mt-1 text-[10px] text-slate-500">Days</div>
              </div>
            </div>
          </Field>
          ...
          {id && masterUnlocked && (
            <div className="md:col-span-2 rounded-xl border border-amber-200 bg-amber-50/60 p-4">
              <div className="font-semibold text-slate-900">
                Administrator Result Correction
              </div>
              <div className="mt-1 text-xs text-slate-600">
                Master Data is unlocked. Result and result date correction is enabled.
              </div>
              ...
              <div className="mt-3 overflow-x-auto rounded-lg border bg-white">
                <table className="w-full min-w-[850px] text-sm">
                  ...
                  <tbody>
                    {form.samples.flatMap((sample, sampleIndex) =>
                      (sample.tests || []).map((test, testIndex) => (
                        <tr key={`${sample.id}-${test.id || testIndex}`} className="border-t">
                          <td className="px-3 py-2">{sample.lab_number || "—"}</td>
                          <td className="px-3 py-2">{sample.sample_type}</td>
                          <td className="px-3 py-2 font-medium">{test.test}</td>
                          <td className="px-3 py-2">
                            <Input
                              value={test.result1 || ""}
                              onChange={(event) =>
                                setForm((current) => {
                                  const samples = [...current.samples];
                                  const tests = [...(samples[sampleIndex].tests || [])];
                                  tests[testIndex] = { ...tests[testIndex], result1: event.target.value };
                                  samples[sampleIndex] = { ...samples[sampleIndex], tests };
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                setForm((current) => {
                                  const samples = [...current.samples];
                                  const tests = [...(samples[sampleIndex].tests || [])];
                                  tests[testIndex] = { ...tests[testIndex], result2: event.target.value };
                                  samples[sampleIndex] = { ...samples[sampleIndex], tests };
                                  return { ...current, samples };
                                })
                              }
                            ...
                            <Input
                              ...
                              onChange={(event) =>
                                setForm((current) => {
                                  const samples = [...current.samples];
                                  const tests = [...(samples[sampleIndex].tests || [])];
                                  tests[testIndex] = { ...tests[testIndex], result_date: event.target.value };
                                  samples[sampleIndex] = { ...samples[sampleIndex], tests };
                                  return { ...current, samples };
                                })
                              }
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <Field label="Sample Type *">
              <div className="flex flex-wrap gap-2">
                {sampleTypesForDataset(sampleDraft.dataset).length === 0 ? (
                  <div className="text-sm text-slate-500">
                    No samples are mapped to this dataset.
                  </div>
                ) : (
                  sampleTypesForDataset(sampleDraft.dataset).map((sample) => {
                    const selected =
                      sampleDraft.sample_type === sample;
                    ...
                    return (
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          updateDraft("sample_type", sample)
                        }
            ...
            >
              <input
                list="sample-quality-options"
                value={sampleDraft.sample_quality || ""}
                onInput={(event) =>
                  updateDraft("sample_quality", event.currentTarget.value)
                }
                onChange={(event) =>
                  updateDraft("sample_quality", event.currentTarget.value)
                }
                ...
                placeholder="Select or enter sample quality"
                className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
              />
              <datalist id="sample-quality-options">
                <option value="Good" />
                <option value="Poor" />
                <option value="Lysed" />
            ...
            >
              <input
                list="transport-condition-options"
                value={sampleDraft.transport_condition || ""}
                onInput={(event) =>
                  updateDraft("transport_condition", event.currentTarget.value)
                }
                onChange={(event) =>
                  updateDraft("transport_condition", event.currentTarget.value)
                }
                ...
                placeholder="Select or enter transport condition"
                className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
              />
              <datalist id="transport-condition-options">
                <option value="Proper cold chain" />
                <option value="Improper cold chain" />
              </datalist>
            </Field>

            {sampleSupportsNvhcp(sampleDraft) && (
              <div className="md:col-span-2 rounded-xl border border-blue-100 bg-blue-50/60 p-3">
                <Field label="NVHCP Programme">
                  <div className="flex gap-5 pt-1 text-sm">
                    <label className="flex items-center gap-2">
                      <input
                        type="radio"
                        name="nvhcp-program"
                        checked={sampleDraft.nvhcp_program === true}
                        onChange={() => updateDraft("nvhcp_program", true)}
                      />
                      Yes
                    </label>
                    <label className="flex items-center gap-2">
                      <input
                        ...
                        name="nvhcp-program"
                        checked={sampleDraft.nvhcp_program !== true}
                        onChange={() => updateDraft("nvhcp_program", false)}
                      />
                      No
                    </label>
                  </div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Shown only for Hepatitis B Virus quantitation and Hepatitis C Virus quantitation samples.
                </p>
              </div>
            )}
