...
export default function DataEntry() {
  ...
  const selectableItems = useMemo(() => {
    const query = String(sampleDraft.testSearch || "")
      .trim()
      .toLowerCase();
    const allowedSet = new Set(testsForDraft);
    ...
    const tests = testsForDraft.map((test) => ({
      key: `test::${test}`,
      name: test,
      kind: "test",
      tests: [],
  ...
  const addSelection = (value) => {
    ...
    setSampleDraft((current) => {
      const next = { ...current };
      const existing = new Set(next.tests.map((test) => test.test));
      let names = [];

      if (value.startsWith("panel::")) {
        const panel = (
          opts.panels_by_dataset?.[next.dataset] || []
        ).find((item) => item.id === value.slice(7));
        names = panel?.tests || [];
      } else {
      ...
      if (!additions.length) {
        toast.info("The selected test or panel is already added");
        return current;
      }

      next.tests = [...next.tests, ...additions];
      next.testSearch = "";
      return next;
    });
  };
              ...
              <div className="mt-2 space-y-3 rounded-xl border border-slate-200 p-3">
                ...
                <div className="overflow-hidden rounded-lg border border-slate-200">
                  <table className="w-full text-sm">
                    <thead className="bg-slate-50">
                      <tr className="text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                        <th className="px-3 py-2">Selected Test</th>
