...
export default function DataEntry() {
  ...
  const selectableItems = useMemo(() => {
    ...
    (
      ...
      // A panel is valid for the selected sample only when every test
      // in the panel is mapped to that sample type. Partial panels are
      ...
      const panelName = String(panel.name || "").trim();
      const dedupeKey = panelId || panelName.toLowerCase();

      if (!dedupeKey || uniquePanels.has(dedupeKey)) return;

      uniquePanels.set(dedupeKey, {
        key: `panel::${panelId}`,
        name: panelName,
        kind: "panel",
        tests: panelTests,
  ...
  }, [opts, sampleDraft, testsForDraft]);
    ...
    setSampleDraft((current) => {
      ...
      if (key === "sample_type") {
        ...
        next.tests = (next.tests || []).filter((test) =>
          allowed.size ? allowed.has(test.test) : true
        );
  ...
  const addSelection = (value) => {
    ...
    setSampleDraft((current) => {
      ...
      const allowed = new Set(testsForDraft);
      ...
      if (value.startsWith("panel::")) {
        ...
        (panel.tests || [])
          .filter((name) => !allowed.size || allowed.has(name))
          .forEach((name) => {
            ...
            if (existing) {
              ...
              if (
                !existing.sources.some(
                  (item) =>
                    item.type === "panel" && item.id === panel.id
                )
              ) {
  ...
  const save = async (event) => {
    ...
    try {
      const samples = form.samples.map((sample) => ({
        ...
        epid_number: sample.epid_number || null,
        sample_type: sample.sample_type,
      ...
      const firstEpidSample = samples.find(
        (sample) => sample.epid_number
      );
      ...
      const payload = {
        ...
        epid_number: firstEpidSample?.epid_number || null,
        samples,
                ...
                <table className="w-full text-sm">
                  <thead className="bg-slate-50">
                    <tr className="text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                      ...
                      <th className="px-3 py-2">EPID Number</th>
              ...
              <Field label="EPID Number">
                <div className="flex">
                  <div className="flex items-center whitespace-nowrap rounded-l border border-r-0 bg-slate-50 px-3 text-sm font-medium text-slate-700">
                    {prefixForDraft || "Select a valid district"}
                  </div>
                  <Input
                    value={suffixForDraft}
                    onChange={(event) =>
                      updateDraft(
                        "epid_number",
                        prefixForDraft
                          ? `${prefixForDraft} ${event.target.value}`.trim()
                          : event.target.value
                      )
                    }
                    disabled={!prefixForDraft}
                    className="rounded-l-none"
                    placeholder="Enter serial number, e.g. 001"
                  />
                </div>
                <p className="mt-1 text-xs text-slate-500">
                  The dataset, state, district code and current two-digit year are generated automatically. Enter only the serial number.
                </p>
              </Field>
            )}
            ...
            {sampleDraft.sample_type && mappingForDraft && (
              <div className="md:col-span-2 rounded-lg bg-blue-50 px-3 py-2 text-xs text-blue-700">
                {mappingForDraft.auto_assign
                  ? "The mapped tests were assigned automatically. Only panels whose complete test set is valid for this sample type are shown."
                  : "Only mapped tests and panels whose complete test set is valid for this sample type are shown."}
              </div>
            )}
