...
export default function DataEntry() {
  ...
  const selectedDataset =
    DATASETS.find((dataset) => dataset.key === form.dataset) || DATASETS[0];

  const datasetMappings = useMemo(
    () => opts.sample_mappings_by_dataset?.[form.dataset] || [],
    [opts, form.dataset]
  );
  ...
  const sampleOptions = useMemo(() => {
    ...
    return Array.isArray(datasetSamples) && datasetSamples.length
      ? datasetSamples
      : opts.sample_type || [];
  }, [datasetMappings, opts, form.dataset]);

  const mappingForSample = (sampleType) =>
    datasetMappings.find(
      (mapping) =>
        String(mapping.sample_type || "").toLowerCase() ===
        String(sampleType || "").toLowerCase()
    );

  const testsForSample = (sample) => {
    const mapping = mappingForSample(sample.sample_type);
    return mapping ? mapping.tests || [] : testOptions;
  };


  const panelOptions = useMemo(
    () => opts.panels_by_dataset?.[form.dataset] || [],
    [opts, form.dataset]
  );
    ...
    const loadData = async () => {
      try {
        ...
        if (id) {
          ...
          setForm({
            dataset: record.dataset || record.dataset_type || "routine",
  ...
  const updateDataset = (dataset) => {
    ...
    setForm((current) => ({
      ...current,
      dataset,
      lab_number: id ? current.lab_number : "",
      epid_number: EPID_DATASETS.has(dataset) ? current.epid_number : "",
  ...
  const updateSample = (sampleIndex, key, value) => {
    setForm((current) => {
      ...
      if (key === "sample_type") {
        const mapping = mappingForSample(value);
  ...
  const addSample = () =>
    setForm((current) => ({
      ...current,
      samples: [...current.samples, newSample()],
    }));
  ...
  const save = async (event) => {
    ...
    if (!form.dataset || !form.name || !form.district) {
    ...
    try {
      ...
      const payload = {
        ...
        epid_number: EPID_DATASETS.has(form.dataset)
      ...
      if (savedRecord.lab_number) {
        toast.success(`Lab Number: ${savedRecord.lab_number}`, {
          duration: 5000,
        });
      }
    } catch (error) {
        ...
        <form onSubmit={save} className="grid grid-cols-1 gap-5 md:grid-cols-2">
          ...
          <Field label="Lab Number">
            <Input
              ...
              value={
                form.lab_number ||
                `${selectedDataset.prefix} - auto generated on save`
              }
              readOnly
          ...
          {EPID_DATASETS.has(form.dataset) && (
            ...
            {form.samples.map((sample, sampleIndex) => (
              ...
              >
                ...
                <div className="grid grid-cols-1 gap-4">
                  ...
                  {sample.sample_type && mappingForSample(sample.sample_type) && (
                    <p className="text-xs text-slate-500">
                      {mappingForSample(sample.sample_type).auto_assign
