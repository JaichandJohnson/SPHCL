...
const formatResult = (testRow) => {
  const result1 = testRow?.result1 || testRow?.result_1 || "";
  const result2 = testRow?.result2 || testRow?.result_2 || "";
...
const getTestRows = (record) => {
  // New format expected from backend:
  // tests: [{ test, result1, result2, resultDate }]
  if (Array.isArray(record.tests) && record.tests.length > 0) {
    return record.tests.map((t, index) => ({
      key: `${record.id}-test-${index}`,
      test: t.test || t.name || "—",
      result: formatResult(t),
      resultDate: t.resultDate || t.result_date || record.result_date || "—",
    }));
  }

  // Temporary compatibility with current/old backend:
  // test: "COVID-19", results: [{ name: "Positive", value: "" }]
  if (record.test) {
    const resultText = Array.isArray(record.results) && record.results.length > 0
      ? record.results
          .filter((r) => r?.name || r?.value)
          .map((r) => [r.name, r.value].filter(Boolean).join(" "))
          .join(", ")
      : "Pending";

    return [{
      key: `${record.id}-legacy-test`,
      test: record.test,
      result: resultText || "Pending",
      resultDate: record.result_date || "—",
    }];
  }

  return [{
    key: `${record.id}-empty-test`,
    test: "—",
    result: "Pending",
    resultDate: record.result_date || "—",
  }];
        ...
        <div className="overflow-x-auto">
          <table data-testid={TABLE.root} className="w-full text-sm zebra">
            ...
            <tbody className="text-slate-700">
              {loading && (
                <tr>
                  <td colSpan={10} className="px-3 py-6 text-center text-slate-500">Loading…</td>
                </tr>
              )}
              ...
              {!loading && data.items.length === 0 && (
                <tr>
                  <td colSpan={10} className="px-3 py-10 text-center text-slate-500">
                    No records found. Create your first entry.
                  </td>
                </tr>
              )}

              {!loading && data.items.map((r) => {
                const testRows = getTestRows(r);

                return testRows.map((testRow, index) => {
                  const isFirstRow = index === 0;

                  return (
                    <tr
                      key={testRow.key}
                      data-testid={isFirstRow ? TABLE.row(r.id) : undefined}
                      className="border-b border-slate-100 hover:bg-blue-50/40"
                    >
                      <td className="px-3 py-2 font-mono text-xs text-slate-900">
                        {isFirstRow ? r.lab_number : ""}
                      </td>
                      ...
                      <td className="px-3 py-2">
                        {isFirstRow ? r.district : ""}
                      </td>
                      <td className="px-3 py-2">
                        {isFirstRow ? r.sample_type : ""}
                      </td>
                      <td className="px-3 py-2">
                        {testRow.test}
                      </td>
                      <td className="px-3 py-2">
                        <span className={testRow.result === "Pending" ? "text-xs text-amber-600 font-medium" : ""}>
                          {testRow.result}
                        </span>
                      </td>
