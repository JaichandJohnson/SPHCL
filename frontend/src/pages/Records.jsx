...
// Lab numbers sort by numeric sequence first, then prefix alphabetically.
const compareLabNumbers = (left, right) => {
  const parse = (value) => {
    const text = String(value || "").trim();
    const match = text.match(/\d+/);
    return { number: match ? Number(match[0]) : Number.POSITIVE_INFINITY, prefix: match ? text.slice(0, match.index).trim() : text, suffix: match ? text.slice(match.index + match[0].length).trim() : "" };
  };
  const a = parse(left);
  const b = parse(right);
  return a.number - b.number || a.prefix.localeCompare(b.prefix, undefined, { sensitivity: "base" }) || a.suffix.localeCompare(b.suffix, undefined, { numeric: true, sensitivity: "base" });
};
...
const sortVal = (record, column) => {
  ...
  return {
    ...
    test: first.test || "",
    additional_result: first.additionalResult || "",
...
export default function Records() {
  ...
  }, [q, selectedDataset, page, pageSize]);
  ...
  useEffect(() => {
    const checkMasterAccess = () => {
      api
        .get("/master/status")
        .then((response) =>
          setMasterUnlocked(Boolean(response.data?.unlocked))
        )
        .catch(() => setMasterUnlocked(false));
    };

    checkMasterAccess();
    window.addEventListener("focus", checkMasterAccess);
    return () =>
      window.removeEventListener("focus", checkMasterAccess);
  }, []);
  ...
  const items = useMemo(() => {
    ...
    return [...(data.items || [])].sort((a, b) => {
      ...
      if (sort.column === "lab_number") {
        comparison = compareLabNumbers(x, y);
      } else if (sort.column === "result") {
  ...
  const deleteRecord = async (record) => {
    const labNumbers = (record.samples || [])
      .map((sample) => sample.lab_number)
      .filter(Boolean)
      .join(", ");

    const confirmed = window.confirm(
      `Administrator action\n\nDelete the entire record for ${
        record.name || "this patient"
      }${labNumbers ? ` (${labNumbers})` : ""}?\n\n` +
        "This permanently removes the record. Lab-number sequences will " +
        "not be rolled back or reused."
    );
  ...
  return (
    <div className="space-y-4" data-testid="records-page">
      ...
      <div className="flex flex-wrap gap-2">
        {RECORD_DATASETS.map((dataset) => {
          const active = selectedDataset === dataset.key;
          return (
            <Button
              key={dataset.key}
              type="button"
              ...
              onClick={() => {
                setSelectedDataset(dataset.key);
                setPage(1);
              }}
              className={
                active
                            ...
                            <div className="inline-flex">
                              ...
                              {masterUnlocked && (
                                <Button
                                  variant="ghost"
                                  size="icon"
                                  title="Delete entire record (Administrator)"
