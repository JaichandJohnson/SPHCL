...
export default function BackupRestore() {
  ...
  const clearTestData = async () => {
    if (clearConfirmation !== "DELETE TEST DATA") {
    ...
    try {
      const response = await api.delete(
        ...
        {
          params: {
            confirm: clearConfirmation,
          },
        }
      );

      toast.success(
        `${response.data.records_deleted} test records deleted. ` +
          "Numbering reset to 1."
      );
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="rounded-2xl border border-red-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <h2 className="font-semibold text-slate-900">
              Clear Test Data
            ...
            <p className="mt-1 text-sm text-slate-500">
              Deletes laboratory records and resets all numbering to 1 while
              retaining Master Data, users, mappings, panels, institutions,
              TAT thresholds and backup configuration.
            </p>
          </div>
          ...
          <Button
            ...
            disabled={busy || restoreBusy || clearBusy}
            className="shrink-0 border-red-300 text-red-600 hover:bg-red-50"
          >
            <WarningCircle size={17} className="mr-2" />
            Clear Test Data
          </Button>
        </div>
      </Card>
      ...
      {clearDialogOpen && (
        <Card className="rounded-2xl border border-red-300 bg-red-50 p-5 shadow-sm">
          <div className="flex items-start gap-3">
            <WarningCircle
              size={28}
              weight="fill"
            ...
            <div className="flex-1">
              ...
              <p className="mt-2 text-sm leading-6 text-slate-700">
                This will permanently delete all current laboratory/patient
                records and restart operational numbering from 1.
                Master Data and configuration will be retained.
              </p>

              <div className="mt-3 rounded-lg border border-red-200 bg-white p-3 text-sm text-slate-700">
                <strong>Safety:</strong> before deleting anything, MDS LIMS
                must successfully create a fresh Google Drive backup of the
                current database. If that backup fails, no data will be
                deleted.
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Button
                  ...
                  disabled={
                    clearBusy ||
                    clearConfirmation !== "DELETE TEST DATA"
                  }
                  className="bg-red-600 text-white hover:bg-red-700"
                >
                  <WarningCircle size={17} className="mr-2" />
